...
<!-- Modal Structure -->
<div id="modal{{ $brand->id }}" class="modal modaldeleteBrand" style="background: transparent;">
  <div class="modal-content">
    ...
    <form action="{{ route('brands.destroy', $brand->id) }}" method="post">
      ...
      <p class="fs-5 text-center py-3">Are you sure to delete brand <b class="text-danger">{{ $brand->brand_name }}</b>?</p>
...
{{-- End Delete Modal --}}
  ...
  <div class="table_hover w-100 d-flex justify-content-between align-items-center border-bottom">
    <a href="{{ route('brands.show',$brand->id) }}" class="w-100 text-decoration-none text-dark py-3 waves-effect">
      <div class="w-100 d-flex justify-content-between">
        <p class="text-center text-truncate m-0" style="width: 5%;">{{ $brand->id }}</p>
        <p class="text-truncate m-0" style="width: 25%;">{{ $brand->brand_name }}</p>
        <p class="text-truncate m-0" style="width: 30%;">{{ $brand->brand_code }}</p>
        <p class="text-truncate m-0" style="width: 40%;">{{ $brand->brand_note }}</p>
      </div>
    </a>
